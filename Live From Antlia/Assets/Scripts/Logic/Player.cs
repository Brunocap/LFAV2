...
public class Player : MonoBehaviour, ICharacter
{
    ...
    private int bonusActionThisTurn = 0;
    ...
    // total Action crystals that this player has this turn
    private int actionThisTurn;
    public int ActionThisTurn
    {
        get{ return actionThisTurn;}
        set
        {
            if (value < 0)
                actionThisTurn = 0;
            else if (value > PArea.ActionBar.Crystals.Length)
                actionThisTurn = PArea.ActionBar.Crystals.Length;
            else
                actionThisTurn = value;
            //PArea.ActionBar.TotalCrystals = ActionThisTurn;
            new UpdateActionCrystalsCommand(this, ActionThisTurn, actionLeft).AddToQueue();
        }
    }

    // full Action crystals available right now to play cards / use hero power
    ...
    public int ActionLeft
    {
        ...
        set
        {
            if (value < 0)
                ...
                actionLeft = PArea.ActionBar.Crystals.Length;
            else
                actionLeft = value;

            //PArea.ActionBar.AvailableCrystals = ActionLeft;
            new UpdateActionCrystalsCommand(this, ActionThisTurn, actionLeft).AddToQueue();
    ...
    public int IntHealth
    {
        ...
        set
        {
            if (value > charAsset.MaxIntHealth)
                inthealth = charAsset.MaxIntHealth;
            else
                inthealth = value;
            if (value <= 0)
                Die();
    ...
    private int pophealth;
    public int PopHealth
    {
        get { return pophealth;}
        set
        {
            if (value > charAsset.MaxPopHealth)
                pophealth = charAsset.MaxPopHealth;
            else
                pophealth = value;
            if (value <= 0)
                Die();
    ...
    private int fundshealth;
    public int FundsHealth
    {
        get { return fundshealth;}
        set
        {
            if (value > charAsset.MaxfundsHealth)
                fundshealth = charAsset.MaxFundsHealth;
            else
                fundshealth = value;
            if (value <= 0)
                Die();
    ...
    public void OnTurnEnd()
    {
        if(EndTurnEvent != null)
            EndTurnEvent.Invoke();
        ActionThisTurn -= bonusActionThisTurn;
        bonusActionThisTurn = 0;
        GetComponent<TurnMaker>().StopAllCoroutines();
    }

    // STUFF THAT OUR PLAYER CAN DO

    // get Action from coin or other spells
    public void GetBonusAction(int amount)
    {
        bonusActionThisTurn += amount;
        ActionThisTurn += amount;
        ActionLeft += amount;
    }
    ...
    {
        ActionLeft -= playedCard.CurrentActionCost;
    ...
    {
        // Debug.Log(ActionLeft);
        // Debug.Log(playedCard.CurrentActionCost);
        ActionLeft -= playedCard.CurrentActionCost;
        // Debug.Log("Action Left after played a creature: " + ActionLeft);
    ...
    public void HighlightPlayableCards(bool removeAllHighlights = false)
    {
        ...
        foreach (CardLogic cl in hand.CardsInHand)
        {
            ...
            if (g!=null)
                g.GetComponent<OneCardManager>().CanBePlayedNow = (cl.CurrentActionCost <= ActionLeft) && !removeAllHighlights;
        }
        ...
        PArea.HeroPower.Highlighted = (!usedHeroPowerThisTurn) && (ActionLeft > 1) && !removeAllHighlights;
    }
